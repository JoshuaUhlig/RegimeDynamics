...
def bin_fpts_2d(
    ...
    if mode == "voxel":
        ...
        xs, ys = voxels[:, 0], voxels[:, 1]
        x0, x1 = xs.min(), xs.max()
        y0, y1 = ys.min(), ys.max()
        grid = np.full((y1 - y0 + 1, x1 - x0 + 1), np.nan)
        for (vx, vy), val in voxel_means.items():
            grid[vy - y0, vx - x0] = val
        filled = np.where(np.isnan(grid), 0.0, grid)
        mask = np.where(np.isnan(grid), 0.0, 1.0)
        smoothed_mask = gaussian_filter(mask, sigma=smooth_sigma)
        smoothed = gaussian_filter(filled, sigma=smooth_sigma) / (smoothed_mask + 1e-12)
        coverage_mask = gaussian_filter(mask, sigma=coverage_sigma)
        smoothed[coverage_mask < 0.05] = np.nan
        ...
        raise ValueError(f"mode must be 'voxel' or 'smooth', got {mode!r}")
...
def plot_fpt_years(
    df, bin_size, symbsize, max_fpt, mode="voxel", smooth_sigma=1.5, coverage_sigma=0.5
):
    intervals = [(1900, 1930), (1930, 1960), (1960, 1990), (1990, 2020)]
    fig, axes = plt.subplots(
        2,
        2,
        figsize=figure_size(width="aps_single", subplots=(2, 2), ratio=0.85),
        sharex=True,
        sharey=True,
        layout="constrained",
    )
    ...
    if colorbar_ref is not None:
        ...
        cbar.set_label("Mean FPT in years", fontsize=12)
...
def test_step_distr(path="all_trajectories/"):
    ...
    fig, ax = plt.subplots(1, 1, figsize=figure_size(width="aps_single"))
    ax.set_xscale("log")
...
def create_hist_one_fig(
    ...
    jumps = np.abs(jumps)
    # step size PDF
    fig, ax = plt.subplots(1, 1, figsize=set_size("aps_double"))
    symbols = ["o", "s"]
...
def create_symmetry_hist(
    ...
    # step size PDF
    fig, ax = plt.subplots(
        1, 2, figsize=figure_size(width="aps_double", subplots=(1, 2))
    )
    ax[0].set_ylabel(r"PDF $\lambda(\mathrm{PC1})$")
    ax[1].set_ylabel(r"PDF $\lambda(\mathrm{PC2})$")
    ax[0].set_xlabel("Step Size $|$PC1$|$")
    ax[1].set_xlabel("Step Size $|$PC2$|$")
    for i in (1, 2):
        pcjumps = jumps[:, i]
        pjumps = pcjumps[pcjumps > 0]
        njumps = np.abs(pcjumps[pcjumps < 0])
        pfraction = len(pjumps) / len(pcjumps) * 100
        nfraction = len(njumps) / len(pcjumps) * 100
        ax[i - 1].tick_params(top=True, right=True, direction="in", which="both")
        ax[i - 1].set_xscale("log")
        ax[i - 1].set_yscale("log")
        _, bins = np.histogram(np.log10(pjumps), bins="fd", density=True)
        hist, bin_edges = np.histogram(pjumps, bins=10**bins, density=True)
        bin_centers = bin_edges[:-1] + (bin_edges[1:] - bin_edges[:-1]) / 2
        bin_centers = bin_centers[hist > 0]
        hist = hist[hist > 0]
        ax[i - 1].plot(
            ...
            mfc="None",
            color=main_color,
            label=f"positive: {round(pfraction, 1)}" + r"\%",
        )
        _, bins = np.histogram(np.log10(njumps), bins="fd", density=True)
        hist, bin_edges = np.histogram(njumps, bins=10**bins, density=True)
        bin_centers = bin_edges[:-1] + (bin_edges[1:] - bin_edges[:-1]) / 2
        bin_centers = bin_centers[hist > 0]
        hist = hist[hist > 0]
        ax[i - 1].plot(
            ...
            mfc="None",
            color=second_color,
            label=f"negative: {round(nfraction, 1)}" + r"\%",
        )
        ax[i - 1].legend(frameon=False)
    fig.tight_layout()

    plt.savefig("figures/hist_symmetry.pdf", bbox_inches="tight")
    plt.close()


def create_pc_correlation(path, eps=0, absolute=False):
    """Plot the correlation between PC1 and PC2 jump sizes with Pearson coefficient.

    Parameters
    ----------
    absolute : bool
        If True, use |deltaPC1| and |deltaPC2| (magnitude correlation).
    """
    jumps, _ = jump_wt_distr(path, eps=eps)
    ...
    r, p_value = pearsonr(pc1, pc2)

    fig, ax = plt.subplots(
        1, 1, figsize=figure_size(width="aps_single", subplots=(1, 1))
    )
    ax.set_title(r"$\rho^2=$" + f"{round(r**2, 2)}")
    ax.tick_params(top=True, right=True, direction="in", which="both")
    ax.set_xlabel(r"$|\Delta\mathrm{PC1}|$" if absolute else "Step Size PC1")
    ax.set_ylabel(r"$|\Delta\mathrm{PC2}|$" if absolute else "Step Size PC2")

    max_bins = 60  # cap FD to avoid tiny sparse bins

    if absolute:
        # Freedman-Diaconis bins in log space, capped
        _, log_edges_x = np.histogram(np.log10(pc1[pc1 > 0]), bins="fd")
        _, log_edges_y = np.histogram(np.log10(pc2[pc2 > 0]), bins="fd")
        if len(log_edges_x) - 1 > max_bins:
            log_edges_x = np.linspace(log_edges_x[0], log_edges_x[-1], max_bins + 1)
        if len(log_edges_y) - 1 > max_bins:
            log_edges_y = np.linspace(log_edges_y[0], log_edges_y[-1], max_bins + 1)
        xbins = 10**log_edges_x
        ybins = 10**log_edges_y
        ax.set_xscale("log")
        ax.set_yscale("log")
    else:
        # Freedman-Diaconis bins in linear space, capped
        xbins = np.histogram_bin_edges(pc1, bins="fd")
        ybins = np.histogram_bin_edges(pc2, bins="fd")
        if len(xbins) - 1 > max_bins:
            xbins = np.linspace(xbins[0], xbins[-1], max_bins + 1)
        if len(ybins) - 1 > max_bins:
            ybins = np.linspace(ybins[0], ybins[-1], max_bins + 1)

    # compute normalised histogram and mask empty bins (avoids white voids in LogNorm)
    H, xedges, yedges = np.histogram2d(pc1, pc2, bins=[xbins, ybins], density=True)
    ...
    img = ax.pcolormesh(
        ...
        H.T,
        cmap="plasma",
        norm=mpl.colors.LogNorm(vmin=H.compressed().min(), vmax=H.max()),
    )
    fig.colorbar(img, ax=ax, label="Density")

    fname = (
        "figures/pc1_pc2_correlation_abs.pdf"
        if absolute
        else "figures/pc1_pc2_correlation.pdf"
    )
...
def plot_ergodicity_breaking(
    path="all_trajectories/",
    min_length=10,
    years=None,
    savepath="figures/",
):
    """Compute and plot the Ergodicity Breaking (EB) parameter over decades.

    For each start year computes individual and ensemble-averaged TAMSD, then
    calculates EB = <delta^4>/<delta^2>^2 - 1 as a function of lag time.
    Saves one TAMSD plot per decade and a summary EB vs. year plot.

    Parameters
    ----------
    path : str
        Directory containing trajectory .dat files.
    min_length : int
        Minimum trajectory length (in years) required for inclusion.
    years : array-like or None
        Start years to analyse. Defaults to 1910-2010 in steps of 10.
    savepath : str
    ...
    country_dict = dict(read_country_table())
    symbols = ["o", "s", "^", "v", "D"]
    # all_ebs[yr_nr, Delta-1] stores EB at that start year and lag time
    all_ebs = np.zeros((len(years), min_length - 1))

    for yr_nr, startyear in enumerate(years):
        avgtamsd = np.zeros(min_length - 1)
        nr_trajectories = 0

        # --- first pass: compute ensemble-averaged TAMSD ---
        for dt in range(1, min_length):
            dx = []
            dy = []
            for filename in os.listdir(path):
                file = os.path.join(path, filename)
                    ...
                    if (
                        ...
                        if trajectory[startindex:, :].shape[0] >= min_length:
                            ...
                            trajectory = trajectory[
                                startindex : startindex + min_length, :
                            ]
                            dx = np.append(
                                dx, (trajectory[dt:, 1] - trajectory[:-dt, 1]) ** 2
                            )
                            dy = np.append(
                                dy, (trajectory[dt:, 2] - trajectory[:-dt, 2]) ** 2
                            )
            avgtamsd[dt - 1] = np.mean(dx) + np.mean(dy)

        # --- second pass: individual TAMSD per country ---
        cmap = matplotlib.colormaps.get_cmap("plasma")
        gradient = np.linspace(0, 1, nr_trajectories)
        colors = [matplotlib.colors.to_hex(cmap(i)) for i in gradient]

        delta2 = np.zeros((min_length - 1, nr_trajectories))
        k = 0
        fig, ax = plt.subplots(1, 1, figsize=figure_size())
        ax.tick_params(top=True, right=True, direction="in", which="both")
        ax.set_title(
            f"{int(startyear)}-{int(startyear + min_length)}, nr_traj={nr_trajectories}"
        )
        ax.set_ylabel(r"TAMSD $\overline{\delta^2(\Delta)}$")
        ax.set_xlabel(r"Lag Time $\Delta$ in years")
        ax.set_yscale("log")
        ...
        for filename in os.listdir(path):
            ...
            if os.path.isfile(file):
                ...
                if trajectory[0, 0] <= startyear and trajectory[-1, 0] > startyear:
                    ...
                    if trajectory[startindex:, :].shape[0] >= min_length:
                        t, msd = tamsd(
                            trajectory[startindex : startindex + min_length, :3]
                        )
                        if len(msd) != min_length - 1:
                            print("something is wrong with length")
                        delta2[:, k] = msd
                        ax.plot(
                            t,
                            msd,
        ...
        ax.plot(
            ...
            avgtamsd,
            color="black",
            label=r"$\langle\overline{\delta^2(\Delta)}\rangle$",
        )

        ax.legend(loc="center left", frameon=False, bbox_to_anchor=(1, 0.5))
        plt.savefig(
            os.path.join(
                savepath, f"EB_tamsd_{startyear}-{startyear + min_length}.pdf"
            ),
            bbox_inches="tight",
        )
        plt.close()
        ...
        ebs = np.zeros(min_length - 1)
        for Delta in np.arange(1, min_length):
            ebs[Delta - 1] = (
                np.mean(delta2[Delta - 1, :] ** 2) / avgtamsd[Delta - 1] ** 2 - 1
            )
        all_ebs[yr_nr, :] = ebs

    # --- summary EB vs. year plot (one line per lag time, coloured by Δ) ---
    lag_times = np.arange(1, min_length)
    cmap = matplotlib.colormaps.get_cmap("plasma")
    norm = mpl.colors.Normalize(vmin=lag_times[0], vmax=lag_times[-1])

    fig, ax = plt.subplots(1, 1, figsize=set_size())
    ax.tick_params(top=True, right=True, direction="in", which="both")
    ax.set_xlabel("Year")
    ax.set_ylabel("EB")
    ax.set_xticks(np.arange(1900, 2021, 10))
    ax.tick_params(axis="x", rotation=45)

    for i, Delta in enumerate(lag_times):
        ax.plot(years + 5, all_ebs[:, i], "o-", color=cmap(norm(Delta)))
    ax.axhline(4, ls="dashed", color="black")
    sm = mpl.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax)
    cbar.set_label(r"Lag Time $\Delta$ in years")
    cbar.set_ticks(lag_times)

    plt.savefig(os.path.join(savepath, "EB_years.pdf"), bbox_inches="tight")
    plt.close()
...
def create_extreme_stepsize_composite(
    ...
    fig, axes_2d = plt.subplots(
        2,
        2,
        figsize=set_size(width=400, subplots=(2, 2), ratio=0.95),
        layout="constrained",
        sharex=True,
        sharey=True,
    )
    ...
    for ax, selections, title, color in zip(axes, selections_list, titles, colors):
        plot_extreme_on_ax(
            ax,
            df,
            features,
    ...
    # only keep axis labels on outer edges
    for ax in axes_2d[0, :]:  # top row: no x-label
        ax.set_xlabel("")
    for ax in axes_2d[:, 1]:  # right column: no y-label
        ax.set_ylabel("")
